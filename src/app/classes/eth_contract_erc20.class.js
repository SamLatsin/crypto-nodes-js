...
const model = "eth_contracts_erc20";

let Eth_contract_erc20 = {
	insert: async function(fields) {
		...
		query = {
			name: "insert eth contract erc20 " + fields.name + fields.address,
			text: query,
			values: data
		};
		const res = await db
	...
	update: async function(fields, id) {
		...
		query = {
			name: "update eth contract erc20 " + id,
			text: query,
			values: data
		};
		const res = await db
	...
	delete: async function(id) {
		...
		query = {
			name: "delete eth contract erc20 " + id,
			text: query,
			values: [id]
		};
		const res = await db
	...
	deleteByTicker: async function(ticker) {
		...
		query = {
			name: "delete eth contract erc20 by ticker " + ticker,
			text: query,
			values: [ticker]
		};
		const res = await db
...
module.exports = Eth_contract_erc20;